        ...
        <div className="md:grid md:grid-cols-2 md:divide-x md:divide-gray-200">
            ...
            <div className="grid grid-cols-7 mt-2 text-sm">
              {days.map((day, dayIdx) => (
                <div key={day.toString()} className={classNames(dayIdx === 0 && colStartClasses[getDay(day)], 'py-1.5')}>
                  ...
                  <div className="w-1 h-1 mx-auto mt-1">{meetings.some((meeting) => isSameDay(parse(meeting.startDatetime, "yyyy-MM-dd'T'HH:mm:ss", new Date()), day)) && <div className="w-1 h-1 rounded-full bg-sky-500"></div>}</div>
                </div>
              ))}
            </div>
          </div>
          <section className="mt-12 md:mt-0 md:pl-14">
            <h2 className="font-semibold text-gray-900">
              Ledige tider
              <time dateTime={format(selectedDay, 'yyyy-MM-dd')}>{format(selectedDay, ' dd. MMM')}</time>
            </h2>
  ...
  return (
    <li className="flex items-center px-4 py-2 space-x-4 group rounded-xl focus-within:bg-gray-100 hover:bg-gray-100">
      <div className="flex-auto">
        ...
        <p className="mt-0.5">
          <time dateTime={startDateTime.toISOString()}>{format(startDateTime, 'hh:mm')}</time>
        </p>
      </div>
    </li>
  );
