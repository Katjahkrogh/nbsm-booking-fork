...
function Overview({ selectedBooking, selectedTreatment }) {
  return (
    <div className="flex flex-col md:items-center mb-20 text-green ">
      ...
      <div className="flex items-top w-full md:w-[500px] ">
        <label
          htmlFor={"politik"}
          className=" cursor-pointer rounded-xl py-4 pl-4 text-sm text-lightGreen relative flex items-end "
        >
          <input
            type="checkbox"
            name="politik"
            id={"politik"}
            className="before:content[''] peer -ml-4 mr-4 mt-2 relative h-5 w-5 cursor-pointer appearance-none rounded-md border border-blue-lighGreen transition-all before:absolute before:top-2/4 before:left-2/4 before:block before:h-10 before:w-10 before:-translate-y-2/4 before:-translate-x-2/4 before:rounded-full before:bg-blue-gray-500 before:opacity-0 before:transition-opacity checked:border-pink-500 checked:bg-pink-500 checked:before:bg-pink-500 hover:before:bg-pink-500 hover:before:opacity-10"
          />
          <div class="pointer-events-none absolute top-[57%] left-[2.5%] -translate-y-2/4 -translate-x-2/4 text-white opacity-0 transition-opacity peer-checked:opacity-100">
            <svg
    ...
    <PrimaryBtn text={"bekræft booking"} />
